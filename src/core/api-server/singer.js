export default {
    ...
    getSingerList() {
        return new Promise((resolve, reject) => {
            ...
            weex.requireModule('stream').fetch({
                method: 'GET',
                url: `/v8/fcg-bin/v8.fcg?g_tk=1928093487&inCharset=utf-8&outCharset=utf-8&notice=0&format=jsonp&channel=singer&page=list&key=all_all_all&pagesize=100&pagenum=1&hostUin=0&needNewCode=0&platform=yqq`,
                type: 'json'
            }, function (ret) {
                if (!ret.ok) {
                    reject(ret.data)
    ...
    getSingerDetail(singerId){
        return new Promise((resolve, reject) => {
            ...
            weex.requireModule('stream').fetch({
                method: 'GET',
                url: `/v8/fcg-bin/fcg_v8_singer_track_cp.fcg?g_tk=1928093487&inCharset=utf-8&outCharset=utf-8&notice=0&format=jsonp&hostUin=0&needNewCode=0&platform=yqq&order=listen&begin=0&num=80&songstatus=1&singermid=${singerId}`,
                type: 'json'
            }, function (ret) {
                if (!ret.ok) {
                    reject(ret.data)
